      ...
      {viewMode === 'list' ? (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
             ...
             <form onSubmit={handleSave} className="space-y-4">
               <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                 ...
                 <div className="space-y-1.5 sm:col-span-1">
                   <label className="text-[10px] font-semibold text-slate-400 uppercase tracking-wider block">Kode Instansi (Bebas/Custom)</label>
                   <div className="flex gap-1.5">
                     <input
                       type="text"
                       required
                       value={kodeInstansi}
                       onChange={(e) => setKodeInstansi(e.target.value.toUpperCase().replace(/[^A-Z0-9-]/g, ''))}
                       placeholder="AUTO GENERATED"
                       className="flex-1 bg-slate-50 border border-slate-200 focus:border-blue-500 rounded-xl py-2.5 px-3.5 text-xs font-mono font-bold text-blue-600 outline-none"
                     />
                     <button
                       type="button"
                       onClick={handleGenerateCode}
                       className="px-2.5 bg-slate-100 border border-slate-200 hover:bg-slate-200 rounded-xl text-[10px] font-bold text-slate-700 transition-all cursor-pointer flex items-center justify-center shrink-0"
                       title="Acak Kode"
                     >
          ...
          <div className="space-y-6 lg:col-span-1 animate-fade-in">
            {/* Instance Code Information Card */}
            <div className="p-6 rounded-2xl border border-slate-200 bg-white backdrop-blur-md space-y-4 animate-fade-in">
              <h3 className="text-sm font-bold text-slate-800 flex items-center gap-2">
                <ShieldAlert className="w-4.5 h-4.5 text-blue-600" />
                <span>Info Integrasi Kode</span>
              </h3>

              <p className="text-slate-450 text-[11px] leading-relaxed">
                Super Admin dapat **bebas menulis kode instansi** buatan sendiri pada form kiri (misalnya: <code className="bg-slate-100 px-1 py-0.5 rounded font-bold text-slate-800 font-mono text-[10px]">SMAN1-BDG</code>).
              </p>
              <p className="text-slate-450 text-[11px] leading-relaxed">
                Kode ini nantinya diinputkan ke dalam **Aplikasi Resmi DIJIT (APK)** milik siswa agar portal siswa terhubung langsung ke instansi terkait secara instan dan eksklusif.
              </p>
            </div>
